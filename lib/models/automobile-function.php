...
function autoMobileMakeNModel(){        
        ...
        if($automobile_year):			
			...
			foreach($results as $result):
				...
				$automobile_make_val[] = get_post_meta( $postId, 'advanced_automobile_make', true );		
				
				$automobile_model[] = get_post_meta( $postId, 'advanced_automobile_model', true );				
				$automobile_model_val[] =  get_post_meta( $postId, 'advanced_automobile_model', true );
			endforeach;
		...
		if($automobile_year):	
			...
			foreach ($get_auto_mobile_year_uns as $key=>$get_auto_mobile_year_unss):
				if (in_array($automobile_year, $get_auto_mobile_year_unss)){
					$make[] = $get_auto_mobile_year_unss['make'];
					$model[] = $get_auto_mobile_year_unss['model'];
				}				
			endforeach;
