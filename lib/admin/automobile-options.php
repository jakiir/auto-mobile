...
function automobile_order_add_page() {
 ...
 /*add_submenu_page( 'automobile_options', 'import', 'Import', 'edit_theme_options', 'automobile_import', 'automobile_import_page');*/
 }
/**
...
/**
 * Output the automobile import page page
 */
/*function automobile_import_page() {
	echo '<div class="wrap">';
	echo '<div id="icon-tools" class="icon32">
	</div>';
	...
	echo '<p>Import</p>';
	echo '<input type="file" name="import_file" id="import_file"/><br><input type="submit" name="import_submit" id="import_submit" class="button button-primary button-large"/>';
...
function admin_make_model(){
        $automobile_make = $_POST['automobile_make'];		
        if($automobile_make):	
			...
			foreach ($get_auto_mobile_year_uns as $key=>$get_auto_mobile_year_unss):
				if (in_array($automobile_make, $get_auto_mobile_year_unss)){
					$models[] = $get_auto_mobile_year_unss['model'];
					$years[] = $get_auto_mobile_year_unss['year'];
				}				
			endforeach;
...
add_action( 'wp_ajax_nopriv_admin_make_model','admin_make_model' );
add_action( 'wp_ajax_admin_make_model','admin_make_model' );
