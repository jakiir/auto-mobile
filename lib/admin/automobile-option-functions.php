...
function edit_attributes(){
    ...
    if($data_keys):
        ...
        $get_mobile_attributes = get_option( $auto_mobile_attributes );
        $get_mobile_make_uns = unserialize($get_mobile_attributes);
        $get_mobile_make_uns[$data_keys] = array(
			'make'    =>   $make_value,
			'model'    =>   $model_val,
			'year'    =>   $year_val
		);
        $item_inform2 = serialize($get_mobile_make_uns);
        update_option( $auto_mobile_attributes, $item_inform2 );
    endif;
    die();
}
add_action( 'wp_ajax_nopriv_edit_attributes','edit_attributes' );
add_action( 'wp_ajax_edit_attributes','edit_attributes' );
...
function add_attributes(){
	...
	$year_val = $_POST['year_val'];
    //$success = false;
    if($make_val !='' && $model_val !='' && $year_val !=''){
        ...
        $attributes_info2 = serialize($attributes_info);
        $auto_mobile_attributes = '_auto_mobile_attributes';
        if ( get_option( $auto_mobile_attributes ) !== false ) {
           ...
           $get_auto_mobile_attributes = get_option( $auto_mobile_attributes );
            ...
            $lastKey = substr(end(array_keys($get_auto_mobile_attributes_uns)), -1);
            $incrLast = intval($lastKey) + 1;
            $item_info2 = array(
				'attribute_'.$incrLast => array(
					'make' => $make_val,
					'model' => $model_val,
					'year' => $year_val
				)
            );
            $itemInfo_result = array_merge($get_auto_mobile_attributes_uns, $item_info2 );
            $item_inform2 = serialize($itemInfo_result);
           update_option( $auto_mobile_attributes, $item_inform2 );

        } else {
            $incrLast = 1;
            add_option( $auto_mobile_attributes, $attributes_info2, '', 'yes' );
        }

		if($incrLast%2 == 0): $class = 'even'; else : $class = 'odd'; endif;
...
add_action( 'wp_ajax_nopriv_add_attributes','add_attributes' );
add_action( 'wp_ajax_add_attributes','add_attributes' );
...
add_action( 'wp_ajax_nopriv_del_attributes','del_attributes' );
add_action( 'wp_ajax_del_attributes','del_attributes' );
